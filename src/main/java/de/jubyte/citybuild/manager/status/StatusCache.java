package de.jubyte.citybuild.manager.status;
...
public class StatusCache {
    ...
    public StatusCache() {
        statusPlayerCache = new ArrayCache<StatusPlayer>()
                .setExpireAfterAccess(30, TimeUnit.MINUTES)
                .setMaxSize(500)
                .registerQuery("byUUID", new CacheQuery<StatusPlayer>() {
                    @Override
                    public StatusPlayer load(Object[] identifiers) {
                        UUID uuid = (UUID) identifiers[0];
                        DatabaseCollection collection = CityBuildV2.getPLUGIN().getStorage().getStatusCollection();
