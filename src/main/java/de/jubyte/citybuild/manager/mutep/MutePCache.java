...
public class MutePCache {
    ...
    public MutePCache() {
        mutepPlayerCache = new ArrayCache<MutepPlayer>()
                .setExpireAfterAccess(30, TimeUnit.MINUTES)
                .setMaxSize(500)
                .registerQuery("byUUID", new CacheQuery<MutepPlayer>() {
                    @Override
                    public MutepPlayer load(Object[] identifiers) {
                        UUID uuid = (UUID) identifiers[0];
                        DatabaseCollection collection = CityBuildV2.getPLUGIN().getStorage().getMutePCollection();
                        ...
                        return new MutepPlayer(uuid, entry.getString("Reason"), entry.getLong("Duration"), entry.getLong("Cooldown"), entry.getUniqueId("Creator"));
                    }
