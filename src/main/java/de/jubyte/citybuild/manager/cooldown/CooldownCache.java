...
public class CooldownCache {
    ...
    public CooldownCache() {
        cooldownCache = new ArrayCache<CooldownPlayer>()
                .setExpireAfterAccess(15,TimeUnit.MINUTES)
                .setMaxSize(500)
                .registerQuery("byUUID", new CacheQuery<CooldownPlayer>() {
                    @Override
                    public CooldownPlayer load(Object[] identifiers) {
                        UUID uuid = (UUID) identifiers[0];
                        DatabaseCollection collection = CityBuildV2.getPLUGIN().getStorage().getcooldownCollection();
                        QueryResultEntry entry = collection.find().where("UUID", uuid).execute().firstOrNull();

                        if(entry == null) {
                            insertCooldownPlayer(uuid);
                            return new CooldownPlayer(uuid, 0, 0, 0);
                        }
                        return new CooldownPlayer(uuid, entry.getLong("Head"), entry.getLong("BreakBlock"), entry.getLong("GiftRank"));
                    }
                    @Override
