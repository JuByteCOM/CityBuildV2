...
public class LibDownloader {
  ...
  public enum Library {
    HTMMLUNIT(
        "https://jubyte.de/download.jubyte.de/PretronicDatabaseQuery.jar",
        "PretronicDatabaseQuery",
        "https://docs.pretronic.net/pretronic-database-query/",
        "DatabaseQuery-1.3.0");
    ...
    if (!(new File(localPath).exists())) {
      ...
      try {
        ...
        CityBuildV2.getPLUGIN()
            .getLogger()
            .severe("An error occured while downloading a required lib.");
        e.printStackTrace();
      }
    }
