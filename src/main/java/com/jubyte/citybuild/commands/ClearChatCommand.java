...
public class ClearChatCommand extends AbstractCommand {
  ...
  public ClearChatCommand() {
    super(
        ConfigData.CONFIG_COMMAND_CHATCLEAR_NAME,
        null,
        "Delete the chat in the event of spam, advertising, insults or other situations.",
        ConfigData.CONFIG_COMMAND_CHATCLEAR_ALIASES);
  }
    ...
    if (commandSender.hasPermission(MessagesData.CLEARCHAT_COMMAND_PERMISSION_USE)) {
      if (strings.length == 0) {
        ...
        Bukkit.broadcastMessage(
            MessagesData.CLEARCHAT_COMMAND_MESSAGE_CLEARED.replace(
