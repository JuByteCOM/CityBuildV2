...
public class TpHereCommand extends AbstractCommand {
  ...
  public TpHereCommand() {
    super(
        ConfigData.CONFIG_COMMAND_TPHERE_NAME,
        null,
        "Teleport a target to you",
        ConfigData.CONFIG_COMMAND_TPHERE_ALIASES);
  }
      ...
      if (player.hasPermission(MessagesData.TPHERE_COMMAND_PERMISSION_USE)) {
        if (strings.length == 1) {
          ...
          if (target != null) {
            ...
            player.sendMessage(
                MessagesData.TPHERE_COMMAND_MESSAGE_TPTARGETTOYOU.replace(
                    "[targetPlayer]", target.getName()));
          } else {
            player.sendMessage(
                MessagesData.TPHERE_COMMAND_MESSAGE_PLAYERNOTFOUND.replace(
