...
public class GiftRankCommand extends AbstractCommand {
  ...
  public GiftRankCommand() {
    super(
        ConfigData.CONFIG_COMMAND_GIFTRANK_NAME,
        null,
        "With this command, you can gift a rank.",
        ConfigData.CONFIG_COMMAND_GIFTRANK_ALIASES);
  }
          ...
          if (targetPlayer != null) {
            ...
            if (cooldownPlayer.hasGiftRankCooldown(player)) {
              ...
              Bukkit.dispatchCommand(
                  ...
                  MessagesData.GIFTRANK_COMMAND_SETTINGS_DISPATCHCOMMAND.replace(
                      "[targetPlayer]", targetPlayer.getName()));
              player.sendMessage(
                  MessagesData.GIFTRANK_COMMAND_MESSAGE_ADDED_RANK.replace(
                      "[targetPlayer]", targetPlayer.getName()));
              cooldownPlayer.setGiftRank();
              targetPlayer.sendMessage(
                  MessagesData.GIFTRANK_COMMAND_MESSAGE_RANK_SET.replace(
                      "[player]", player.getName()));
              if (MessagesData.GIFTRANK_COMMAND_SETTING_KICK)
                targetPlayer.kickPlayer(
                    MessagesData.GIFTRANK_COMMAND_MESSAGE_KICK_SCREEN.replace(
              ...
              player.sendMessage(
                  MessagesData.GIFTRANK_COMMAND_MESSAGE_COOLDOWN
