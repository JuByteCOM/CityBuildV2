...
public class StartkickCommand extends AbstractCommand {
  ...
  public StartkickCommand() {
    super(
        ConfigData.CONFIG_COMMAND_STARTKICK_NAME,
        null,
        "Let the community decide if a player should be kicked.",
        ConfigData.CONFIG_COMMAND_STARTKICK_ALIASES);
  }
  ...
  public boolean onCommand(
    ...
    if (targetPlayer == null) {
      commandSender.sendMessage(
          MessagesData.STARTKICK_COMMAND_MESSAGE_TARGET_PLAYER_OFFLINE.replace(
    ...
    if (MessagesData.STARTKICK_COMMAND_SETTING_KICK_BYPASS) {
      if (targetPlayer.hasPermission(MessagesData.STARTKICK_COMMAND_PERMISSION_KICK_BYPASS)) {
        commandSender.sendMessage(
            MessagesData.STARTKICK_COMMAND_MESSAGE_KICK_BYPASS.replace(
    ...
    if (commandSender instanceof Player player) {
      ...
      if (!commandSender.hasPermission(MessagesData.STARTKICK_COMMAND_PERMISSION_TIME_BYPASS)) {
        if (startKickPlayer.hasCooldown(player)) {
          ...
          commandSender.sendMessage(
              MessagesData.STARTKICK_COMMAND_MESSAGE_COOLDOWN
            ...
            .scheduleSyncRepeatingTask(
                ...
                () -> {
                    ...
                    if (i != 0 && i != 1) {
                      if (counter == i) {
                        for (Player players : Bukkit.getOnlinePlayers()) {
                          players.sendMessage(
                              MessagesData.STARTKICK_COMMAND_MESSAGE_COUNTER.replace(
                      ...
                      if (MessagesData.STARTKICK_COMMAND_SETTING_PLAY_SOUND) {
                        if (counter == i) {
                          for (Player players : Bukkit.getOnlinePlayers()) {
                            players.playSound(
                                players.getLocation(),
                                MessagesData.STARTKICK_COMMAND_SETTING_COUNTER_SOUND,
                                1F,
                                1F);
                          }
                        }
                  ...
                  if (counter == 30) {
                    if (MessagesData.STARTKICK_COMMAND_SETTING_PLAY_SOUND) {
                      for (Player players : Bukkit.getOnlinePlayers()) {
                        players.playSound(
                            players.getLocation(),
                            MessagesData.STARTKICK_COMMAND_SETTING_START_SOUND,
                            1F,
                            1F);
                      }
                    }
                  ...
                  if (counter == 1) {
                    for (Player players : Bukkit.getOnlinePlayers()) {
                      players.sendMessage(
                          MessagesData.STARTKICK_COMMAND_MESSAGE_LASTSECOND_COUNTER);
                    }
                  }
                  if (counter == 0) {
                    if (MessagesData.STARTKICK_COMMAND_SETTING_PLAY_SOUND) {
                      for (Player players : Bukkit.getOnlinePlayers())
                        players.playSound(
                            players.getLocation(),
                            MessagesData.STARTKICK_COMMAND_SETTING_STARTKICK_SOUND,
                            1F,
                            1F);
                    }
  ...
  public void startStartKick(Player targetPlayer) {
    ...
    if (!plugin.getVOTING_YES().isEmpty()) {
      for (Player all : Bukkit.getOnlinePlayers()) all.sendMessage("§a" + yes_string);
    }
    if (!plugin.getVOTING_NO().isEmpty()) {
    ...
    if (yes_int > no_int) {
      for (Player all : Bukkit.getOnlinePlayers())
        all.sendMessage(
            MessagesData.STARTKICK_COMMAND_MESSAGE_PLAYER_KICKED
                .replace("[targetPlayer]", targetPlayer.getDisplayName())
                .replace("[yesVotes]", Integer.toString(yes_int))
                .replace("[noVotes]", Integer.toString(no_int)));
      ...
      for (Player all : Bukkit.getOnlinePlayers())
        all.sendMessage(
            MessagesData.STARTKICK_COMMAND_MESSAGE_PLAYER_NOT_KICKED
                .replace("[targetPlayer]", targetPlayer.getDisplayName())
                .replace("[yesVotes]", Integer.toString(yes_int))
                .replace("[noVotes]", Integer.toString(no_int)));
    }
