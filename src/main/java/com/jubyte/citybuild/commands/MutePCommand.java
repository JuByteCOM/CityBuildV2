...
public class MutePCommand extends AbstractCommand {
  ...
  public MutePCommand() {
    super(
        ConfigData.CONFIG_COMMAND_MUTEP_NAME,
        null,
        "Player can mute other player temporary.",
        ConfigData.CONFIG_COMMAND_MUTEP_ALIASES);
  }
        ...
        if (strings.length >= 2) {
          ...
          if (mutepPlayer.hasCooldown(player)) {
            ...
            if (targetPlayer != null) {
                ...
                if (!targetPlayer.hasPermission(
                  ...
                  if (!mutepTargetPlayer.playerIsMutep()) {
                    ...
                    for (Player players : Bukkit.getOnlinePlayers()) {
                      players.sendMessage(
                          MessagesData.MUTEP_COMMAND_MESSAGE_MUTE_SUCCESFUL
                              ...
                              .replace(
                                  "[duration]",
                                  Integer.toString(
                                      MessagesData.MUTEP_COMMAND_SETTINGS_DURATION / 60)));
                    }
                    reasons = "";
                  } else {
                    player.sendMessage(
                        MessagesData.MUTEP_COMMAND_MESSAGE_PLAYER_IS_MUTED.replace(
                            "[targetPlayer]", targetPlayer.getDisplayName()));
                  }
                } else {
                  player.sendMessage(
                      MessagesData.MUTEP_COMMAND_MESSAGE_MUTE_BYPASS.replace(
                          "[targetPlayer]", targetPlayer.getDisplayName()));
                }
              } else {
              ...
              player.sendMessage(
                  MessagesData.MUTEP_COMMAND_MESSAGE_TARGET_PLAYER_OFFLINE.replace(
            ...
            player.sendMessage(
                MessagesData.MUTEP_COMMAND_MESSAGE_COOLDOWN
