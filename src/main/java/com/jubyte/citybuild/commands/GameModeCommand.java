...
public class GameModeCommand extends AbstractCommand {
  ...
  public GameModeCommand() {
    super(
        ConfigData.CONFIG_COMMAND_GAMEMODE_NAME,
        null,
        "For switching gamemodes of players",
        ConfigData.CONFIG_COMMAND_GAMEMODE_ALIASES);
  }
    ...
    if (strings.length == 1) {
      ...
      if (commandSender.hasPermission(MessagesData.GAMEMODE_COMMAND_PERMISSION_USE_OTHER)) {
        ...
        if (target != null) {
          ...
          commandSender.sendMessage(
              MessagesData.GAMEMODE_COMMAND_MESSAGE_PLAYER_NOT_FOUND.replace(
                  "[targetPlayer]", strings[1]));
        }
      } else {
