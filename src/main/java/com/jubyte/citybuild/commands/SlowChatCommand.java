...
public class SlowChatCommand extends AbstractCommand {
  ...
  public SlowChatCommand() {
    super(
        ConfigData.CONFIG_COMMAND_SLOWCHAT_NAME,
        null,
        "If there is spam or other situations, slow down the chat.",
        ConfigData.CONFIG_COMMAND_SLOWCHAT_ALIASES);
  }
      ...
      if (strings.length == 0) {
        if (SLOWCHAT_STATUS) {
          for (Player all : Bukkit.getOnlinePlayers()) {
            all.sendMessage(
                MessagesData.SLOWCHAT_COMMAND_MESSAGE_DEACTIVATED.replace(
          ...
          for (Player all : Bukkit.getOnlinePlayers()) {
            all.sendMessage(
                MessagesData.SLOWCHAT_COMMAND_MESSAGE_ACTIVATED.replace(
                    "[player]", commandSender.getName()));
