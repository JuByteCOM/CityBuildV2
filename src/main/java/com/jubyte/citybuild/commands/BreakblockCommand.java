...
public class BreakblockCommand extends AbstractCommand {
  ...
  public BreakblockCommand() {
    super(
        ConfigData.CONFIG_COMMAND_BREAKBLOCK_NAME,
        null,
        "Remove indestructible blocks.",
        ConfigData.CONFIG_COMMAND_BREAKBLOCK_ALIASES);
  }
        ...
        if (player.hasPermission(MessagesData.BREAKBLOCK_COMMAND_PERMISSION_USE)) {
          ...
          if (strings.length == 0) {
            if (!cooldownPlayer.hasBreakblockCooldown(player)) {
              ...
              player.sendMessage(
                  MessagesData.BREAKBLOCK_COMMAND_MESSAGE_COODLOWN
            ...
            if (strings[0].equalsIgnoreCase("confirm")) {
              if (breackBlockPlayers.contains(player)) {
                ...
                if (MessagesData.BREAKBLOCK_COMMAND_SETTINGS_AVIABLE_WORLDS.stream()
                    .filter(world -> player.getWorld().getName().equalsIgnoreCase(world))
                ...
                if (breakAllowed) {
                  if (!MessagesData.BREAKBLOCK_COMMAND_SETTINGS_DISABLES_BLOCK_HEIGHTS.stream()
                      .filter(y -> block.getLocation().getBlockY() == y)
                      .collect(Collectors.toSet())
                      .isEmpty()) {
                    breakAllowed = false;
                    player.sendMessage(
                        MessagesData.BREAKBLOCK_COMMAND_MESSAGE_DISABLED_BLOCK_HEIGHT.replace(
                            "[blockHeight]", String.valueOf(block.getLocation().getBlockY())));
                  }
                }
                if (breakAllowed) {
                  if (MessagesData.BREAKBLOCK_COMMAND_SETTINGS_BLOCKED_BLOCKS.stream()
                      .filter(material -> block.getType() != Material.matchMaterial(material))
                      .collect(Collectors.toSet())
                      .isEmpty()) {
                    breakAllowed = false;
                ...
                if (breakAllowed) {
                  if (Bukkit.getServer().getPluginManager().getPlugin("PlotSquared") == null) {
                    ...
                    player.sendMessage(
                        MessagesData.BREAKBLOCK_COMMAND_MESSAGE_CONFIRM_BLOCK_REMOVED);
                    ...
                    if (Arrays.stream(
                            plotAPI.getPlotSquared().getPlotAreaManager().getAllPlotAreas())
                        .filter(y -> block.getWorld().getName().equalsIgnoreCase(String.valueOf(y)))
                        .collect(Collectors.toSet())
                        .isEmpty()) {
                      breackBlockPlayers.remove(player);
                      player.sendMessage(
                      ...
                      if (PlotUtilsV6.getPlot(player.getTargetBlock(null, 5).getLocation())
                          != null) {
                        if (PlotUtilsV6.getPlot(block.getLocation())
                          ...
                          player.sendMessage(
                              MessagesData.BREAKBLOCK_COMMAND_MESSAGE_ISNT_PLOT_OWNER);
                        }
                      } else {
                        player.sendMessage(
                            MessagesData.BREAKBLOCK_COMMAND_MESSAGE_BLOCK_ISNT_ON_PLOT);
                      }
                    }
                  }
                }
